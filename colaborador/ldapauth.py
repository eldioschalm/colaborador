...
class LDAPBackend(object):
    ...
    def _create_user_object(self, username, password):
        ...
        """
        #if all users have right of staff
        # return User(username=username, password=password, is_staff=True)
        return User(username=username, password=password, is_staff=False)
    ...
    def _update_user(self, l, user):
        ...
        hold = l.search_s(self.settings['LDAP_SEARCHDN'], self.settings['LDAP_SCOPE'], filter)
        if len(hold) < 1:
        ...
        fulln = self.settings['LDAP_FULL_NAME'] or None
